...
Локально используется `docker-compose.local.yml` (порты 8080/5009). `start.bat` создаёт `.env`, скачивает или собирает компоненты и открывает http://localhost:8080.
