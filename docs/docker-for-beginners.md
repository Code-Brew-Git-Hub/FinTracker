...
### Вариант 0: скрипт (самый простой на Windows)

В папке проекта дважды щёлкните `start.bat` или в PowerShell:

```powershell
.\start.bat
```

Скрипт создаст `.env` при необходимости, скачает образы и откроет сайт в браузере. Остановка: `stop.bat`.

Сборка из исходников вместо GHCR: `.\start.ps1 -Build`
